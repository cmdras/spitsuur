...
class Board(object):
    def __init__(self, width, board_level, direction):
        ...
        self.height = width
        if direction == 0:
            board_level = "vehiclesBF/" + board_level + ".txt"
            self.vehicles = start_vehicles(board_level, width)
            self.all_positions = {}
            self.x = ''
        if direction == 1:
            board_level = "vehiclesBF/" + board_level + "BF_End.txt"
            self.vehicles = start_vehicles(board_level, width)
            self.all_positions = {}
            self.x = ''
...
def astar_solver(table_begin, table_end, width):
    i_begin = 0
    i_end = 0
        ...
        for child in children_begin:
            ...
            if len(closedset_begin) > 2:
                if y in closedset_end:
                    game_win_BD(start_time, current_begin, current_end, width)
...
def game_win_BD(tijd, stappen_begin, stappen_end, wijd):
    print "You win!"
